...
SKIRToutputFilePath = params['OutputFilepaths']['SKIRToutputFilePath'].format(simPath=simPath) # Path where the SKIRT output files will be stored
...
def preprocess(snapList):
    ...
    for snap in snapList:
        ...
        for idx, ID in enumerate(halo_IDs):
            ...
            subprocess.run(['python', f'{dir_path}/pythonScripts/saveSKIRTinput.py', str(snap), str(ID), txtFilePath, SKIRTinputFilePath])
